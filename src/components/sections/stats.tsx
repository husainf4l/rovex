...
export function Stats() {
  return (
    <section id="about" className="bg-background py-28 border-t border-border">
      <div className="mx-auto max-w-[980px] px-5">
        <div className="mb-16 text-center">
          ...
          <div className="grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-border bg-border/50 lg:grid-cols-4">
          {stats.map((stat) => (
            <div
              key={stat.label}
              className="flex flex-col items-center justify-center gap-1 bg-background px-8 py-12 text-center"
            >
              <p className="text-[48px] font-extrabold leading-none tracking-[-0.03em] text-foreground sm:text-[56px]">
                {stat.value}
              </p>
              <p className="mt-2 text-[17px] font-semibold text-foreground">
                {stat.label}
              </p>
              <p className="text-[13px] text-muted-foreground">{stat.sub}</p>
            </div>
          ))}
        </div>
      </div>
